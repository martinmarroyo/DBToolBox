...
def get_alchemy_engine(server_name:str):
    ...
    """
    try:
        address = (f"postgresql+psycopg2://{config['USER']}"
...
def get_alchemy_connection(engine=None,server_name=None,stream=False,max_row_buffer=100):
    """
    ...
    try:
        if engine is not None:  
            ...
            try:
                ...
                raise
        if stream:
            conn = conn.execution_options(stream_results=stream, 
...
def db_insertion(data: pd.DataFrame
                ,server_name: str
                ,schema_name: str
                ,table_name: str
                ,engine=None
...
def connect_db():
    ...
    """
    connection = db_connection(config['USER']
                              ,config['PWD']
                              ,config['SERVER']
                              ,config['PORT']
                              ,config['DB'])
...
def insert_nursery(data:pd.DataFrame
                  ,schema:str
                  ,table:str
                  ,engine=None
                  ,chunksize=1000
    ...
    """
    try:
        db_insertion(data=data
                    ,server_name=config['DEV_SERVER']
                    ,schema_name=schema
                    ,table_name=table
                    ,engine=engine
                    ,chunksize=chunksize
...
def insert_garden(data:pd.DataFrame
                  ,schema:str
                  ,table:str
                  ,engine=None
                  ,chunksize=1000
    ...
    try:
        db_insertion(data=data
                    ,server_name=config['STAGE_SERVER']
                    ,schema_name=schema
                    ,table_name=table
                    ,engine=engine
                    ,chunksize=chunksize
        ...
        print(f"Error occurred while trying to insert to garden: {str(e)}")
        raise
    

def insert_greenhouse(data:pd.DataFrame
                  ,schema:str
                  ,table:str
                  ,engine=None
                  ,chunksize=1000
    ...
    try:
        db_insertion(data=data
                    ,server_name=config['PROD_SERVER']
                    ,schema_name=schema
                    ,table_name=table
                    ,engine=engine
                    ,chunksize=chunksize
        ...
        print(f"Error occurred while trying to insert to greenhouse: {str(e)}")
        raise